...
class CapturingRecognitionModel final : public RecognitionProbabilityModel {
public:
    explicit CapturingRecognitionModel(std::vector<RecognitionModelOutput> outputs = {})
    ...
    bool Predict(std::span<const float> features, RecognitionModelOutput& output,
                 std::string*) override {
...
private:
    std::vector<RecognitionModelOutput> m_outputs;
};

RecognitionModelPackage TestPackage() {
...
RecognitionModelOutput Output(float gunshot, float footstep,
...
TEST(RecognitionFeatures, StreamingChunksMatchWholeInput) {
...
TEST(RecognitionFeatures, ChannelSwapHasExpectedSpatialSymmetry) {
...
TEST(RecognitionFeatures, MatchesPythonStereoOnsetGoldenVector) {
...
TEST(SoundRecognizer, LeftPadsStartupContextAndInfersFirstFeature) {
...
TEST(SoundRecognizer, AppliesPeakSpacingAndCalibratedOnset) {
    std::vector<RecognitionModelOutput> outputs(15, Output(0.1f, 0.1f));
...
TEST(SoundRecognizer, AppliesRuntimeTuningOnNextAudioBlock) {
    std::vector<RecognitionModelOutput> outputs(15, Output(0.1f, 0.1f));
    outputs[1] = Output(0.4f, 0.1f);
    outputs[2] = Output(0.9f, 0.1f);
    auto model = std::make_shared<CapturingRecognitionModel>(outputs);
    RecognitionModelPackage package = TestPackage();
    auto tuningStore = std::make_shared<RecognitionRuntimeTuningStore>(
    ...
    SoundRecognizer recognizer(model, package, {}, tuningStore);
...
TEST(SoundRecognizer, SuppressesSelfEventsFromRealtimeCallback) {
    std::vector<RecognitionModelOutput> outputs(15, Output(0.1f, 0.1f));
    outputs[2] = Output(0.9f, 0.1f, {0.98f, 0.01f, 0.01f});
    auto model = std::make_shared<CapturingRecognitionModel>(outputs);
    size_t callbackCount = 0;
    SoundRecognizer recognizer(model, TestPackage(),
                            [&](const SoundEvent&) { ++callbackCount; });
    const size_t pcmFrames = 1024 + 28 * 240;
    std::vector<float> silence(pcmFrames * 2, 0.0f);
    const auto events = recognizer.PushInterleaved(silence.data(), pcmFrames);
    ASSERT_EQ(events.size(), 1u);
...
TEST(SoundRecognizer, FlushSupportsZeroLookaheadAndCarriesStreamGeneration) {
    std::vector<RecognitionModelOutput> outputs{Output(0.9f, 0.1f), Output(0.1f, 0.1f)};
    auto model = std::make_shared<CapturingRecognitionModel>(outputs);
    RecognitionModelPackage package = TestPackage();
    package.peakLookaheadFrames = 0;
    SoundRecognizer recognizer(model, package);
...
TEST(RecognitionModelPackage, ValidatesExportContractAndChecksum) {
    ...
    bool hashOk = false;
    const std::string hash = ComputeFileSha256(modelPath, &hashOk);
    ASSERT_TRUE(hashOk);
    ...
    RecognitionModelPackage package;
    std::string error;
    EXPECT_TRUE(RecognitionModelPackage::Load(root, package, &error)) << error;
