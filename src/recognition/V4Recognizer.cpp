...
StereoOnsetFeatureExtractor::Config FeatureConfig(const V4ModelPackage& package) {
...
SoundClass V4Class(size_t index) {
...
V4Recognizer::V4Recognizer(std::shared_ptr<V4ProbabilityModel> model,
    ...
    if (!m_model || m_model->InputFrames() != m_package.contextFrames ||
        ...
        throw std::invalid_argument("V4 model shape is incompatible with its package");
    }
...
void V4Recognizer::RunInference(const StereoOnsetFeatureFrame& frame,
                                std::vector<V4SoundEvent>& events) {
...
const V4Recognizer::TracePoint& V4Recognizer::TraceAt(uint64_t index) const {
    if (index < m_traceStartIndex || index >= m_traceStartIndex + m_trace.size()) {
        throw std::out_of_range("V4 probability trace index was pruned too early");
    }
    return m_trace[static_cast<size_t>(index - m_traceStartIndex)];
}

void V4Recognizer::EvaluatePeak(uint64_t index, uint64_t rightIndex,
                                std::vector<V4SoundEvent>& events) {
...
void V4Recognizer::ConsiderCandidate(size_t classIndex, uint64_t sample,
...
void V4Recognizer::EmitPending(size_t classIndex, uint64_t deliveredSample,
                               std::vector<V4SoundEvent>& events) {
    PendingCandidate& pending = m_pending[classIndex];
    if (!pending.active) return;
    V4SoundEvent event;
    event.soundClass = V4Class(classIndex);
...
void V4Recognizer::ProcessReadyPeaks(std::vector<V4SoundEvent>& events, bool flush) {
    const uint64_t lookahead = m_package.peakLookaheadFrames;
...
std::vector<V4SoundEvent> V4Recognizer::Flush() {
    std::vector<V4SoundEvent> events;
...
void V4Recognizer::Publish(const std::vector<V4SoundEvent>& events) {
...
void V4Recognizer::OnAudio(const AudioBlockView& block) {
    if (block.sampleRate != 48000 || block.channels != 2 ||
        block.interleaved.size() < block.frameCount * 2) {
        m_lastError = "V4Recognizer requires 48 kHz interleaved stereo PCM";
        return;
    }
