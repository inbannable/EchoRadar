...
/// Policy values that are safe to change while recognition is running.
///
...
struct RecognitionRuntimeTuning {
    ...
    friend bool operator==(const RecognitionRuntimeTuning&, const RecognitionRuntimeTuning&) = default;

    static RecognitionRuntimeTuning FromPackage(const RecognitionModelPackage& package) {
    ...
    void ApplyTo(RecognitionModelPackage& package) const {
        const RecognitionRuntimeTuning safe = Clamp(*this);
    ...
    static RecognitionRuntimeTuning Clamp(RecognitionRuntimeTuning tuning) {
...
class RecognitionRuntimeTuningStore {
public:
    explicit RecognitionRuntimeTuningStore(RecognitionRuntimeTuning initial)
        : m_default(RecognitionRuntimeTuning::Clamp(initial)),
          m_value(m_default) {}

    RecognitionRuntimeTuning Snapshot() const {
        std::lock_guard<std::mutex> lock(m_mutex);
        return m_value;
    }

    void Update(const RecognitionRuntimeTuning& tuning) {
        std::lock_guard<std::mutex> lock(m_mutex);
        m_value = RecognitionRuntimeTuning::Clamp(tuning);
    }
...
private:
    RecognitionRuntimeTuning m_default;
    RecognitionRuntimeTuning m_value;
