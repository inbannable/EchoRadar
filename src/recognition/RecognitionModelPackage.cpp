...
bool RecognitionModelPackage::Load(const std::filesystem::path& directoryPath,
    ...
    if (!input) return Fail(error, "Missing recognition model metadata: " + metadataPath.string());
    ...
    if (!ReadU32(values, "package_version", packageVersion) || packageVersion != 4) {
        return Fail(error, "Recognition model package_version must be 4");
    }
    ...
    if (package.modelVersion.empty() || package.modelSha256.empty() || modelFile.empty()) {
        return Fail(error, "Recognition model metadata is missing version, file, or SHA-256");
    }
    const std::filesystem::path modelFilename(modelFile);
    if (modelFilename.is_absolute() || modelFilename.has_parent_path()) {
        return Fail(error, "Recognition model file must be inside its package directory");
    }
    package.modelPath = directoryPath / modelFilename;
    if (!std::filesystem::is_regular_file(package.modelPath)) {
        return Fail(error, "Recognition ONNX model is missing: " + package.modelPath.string());
    }
    ...
    if (!ReadU32(values, "sample_rate", package.sampleRate) ||
        ...
        return Fail(error, "Recognition preprocessing, class, source, or event contract is incompatible");
    }
    ...
    if (!ReadFloat(values, "pcen_smoothing", package.pcenSmoothing) ||
        ...
        return Fail(error, "Recognition PCEN metadata is incompatible");
    }
    if (!ReadFloat(values, "scene_activity_cutoff", package.sceneActivityCutoff) ||
        ...
        return Fail(error, "Recognition scene/source/peak policy is missing");
    }
    uint32_t pulseMs = 0;
    if (!ReadU32(values, "pulse_ms", pulseMs) || pulseMs == 0 ||
        ...
        return Fail(error, "Recognition scene/source/peak policy is invalid");
    }
    ...
    for (size_t index = 0; index < names.size(); ++index) {
        ...
        if (!ReadFloat(values, "threshold_quiet_" + names[index], package.quietThresholds[index]) ||
            ...
            spacingMs == 0) {
            return Fail(error, "Recognition calibrated policy is invalid for " + names[index]);
        }
    ...
    if (!hashOk || actualHash != package.modelSha256) {
        return Fail(error, "Recognition model SHA-256 does not match model.json");
    }
